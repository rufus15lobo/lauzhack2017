...
namespace SkyScanner.Services
{
    ...
    public sealed class Scanner : IScanner
    {
        ...
        public async Task<List<AnywherePlace>> QueryLocationById(string origin, string destination, string outboundPartialDate,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return await QueryData<LocationById, AnywherePlace>(
                new LocationById(_apiKey, new LocationByIdSettings(origin, destination, outboundPartialDate)),
                cancellationToken);
        }
        /// <summary>
        /// The location autosuggest service can be used to get a list of places (with corresponding IDs) that
